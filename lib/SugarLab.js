...
(function (sugarLab) {
    ...
    sugarLab.Game = function (config) {
        ...
        me.renderer = PIXI.autoDetectRenderer(me.canvas.width, me.canvas.height, {view: me.canvas, antialiasing: false});
        ...
        * @param {sugarLab.Vec2} screenSize New size for the screen
        */
        me.resizeScreen = function (screenSize) {
            me.canvas.width = screenSize.x;
            me.canvas.height = screenSize.y;
            me.screenSize = screenSize.clone();
            me.screenLocation = new sugarLab.Vec2(me.canvas.offsetLeft, me.canvas.offsetTop);
            console.log(me.canvas.width, me.canvas.height);
            me.renderer = PIXI.autoDetectRenderer(me.canvas.width, me.canvas.height, {view: me.canvas});
        };
    ...
    sugarLab.AssetCollection = function (filePath, game, loadCallback) {
        ...
        var assetsFileLoaded = function () {
            ...
            for (var i = 0; i < me.assets.images.length; i++) {
                ...
                me.images[me.images.length - 1].texture = PIXI.Texture.fromImage(me.images[me.images.length - 1].src);
            }
         ...
         */
        this.getTexture = function (query) {
            for (var i = 0; i < me.images.length; i++) {
                if (me.images[i].src.indexOf(query) !== -1) {
                    return me.images[i].texture;
                }
            }
            return null;
        };
     ...
     * @constructor
     */
    sugarLab.Scene = function (name, entities, readyScript, game) {
        var me = this,
        ...
        me.name = name;
        me.game = game;
        me.stage = new PIXI.Stage(0xFFFFFF);
        me.camera = new PIXI.DisplayObjectContainer();
        me.camera.position = new SL.Vec2(0, 0);
        me.entities = [];
        ...
        me.draw = function (sctx) {
            ...
            me.game.renderer.render(me.stage);
        };
        ...
        me.addEntity = function (entity) {
            ...
            entity.sprite ? me.camera.addChild(entity.sprite) : null;
        ...
        me.removeEntity = function (entity) {
            ...
            entity.sprite ? me.camera.removeChild(entity.sprite) : null;
            for (var i = 0; i < entities.length; i++) {
    ...
    */
    ...
    /**
     * Figures the rotation distance in degrees
     *
     * @param rotationFrom Rotation of object to rotate
     * @param rotationTo Target rotation
     * @returns {Number} Returns 1 or -1
     */
    sugarLab.rotationDistance = function (rotationFrom, rotationTo) {
        return Math.abs(sugarLab.wrapAngle(rotationFrom) - sugarLab.wrapAngle(rotationTo));
    };
